  ...
  Model = (function(_super) {
    ...
    */
    ...
    Model.prototype._getProperty = function(property, next) {
      var fetchable, i, isVirtual, key, onFetch, props, v, _i, _len,
        _this = this;
      ...
      for (i = _i = 0, _len = props.length; _i < _len; i = ++_i) {
        key = props[i];
        if (fetchable = this.schema.fields.get(props.slice(0, i + 1).join("."))) {
          break;
        }
      }
      if (!fetchable) {
      ...
      this._fetchedWatched[key] = true;
      if (isVirtual) {
        v = this.get(fetchable.property);
        if (v && (v.__isModel || v.__isCollection)) {
          return v.fetch(onFetch);
        } else {
          return fetchable.fetch(payload.model(this).method("GET").data, onFetch);
        }
      } else {
        if ((this.get(property) != null) && !(fetchable != null ? fetchable.isVirtual() : void 0)) {
          return onFetch();
        }
        return this._throttledFetch(onFetch);
      }
    };
