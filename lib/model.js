// Generated by CoffeeScript 1.6.2
(function() {
  var Model, Payload, bindable,
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };

  bindable = require("bindable");

  Payload = require("./payload");

  Model = (function(_super) {
    __extends(Model, _super);

    /*
    */


    Model.prototype.__isModel = true;

    /*
    */


    function Model(schema) {
      this.schema = schema;
      Model.__super__.constructor.call(this, {});
      this._changed = {};
      this._bindFields();
    }

    /*
    */


    Model.prototype.isNew = function() {
      return !this.has("_id");
    };

    /*
    */


    Model.prototype.hasChanged = function() {
      return !!Object.keys(this._changed);
    };

    /*
    */


    Model.prototype._set = function(key, value) {
      return Model.__super__._set.call(this, key, this.schema.map(key, value));
    };

    /*
    */


    Model.prototype.flushChanged = function() {
      var ch, changed, key;

      ch = this._changed;
      this._changed = {};
      changed = [];
      for (key in ch) {
        changed.push(ch[key]);
      }
      return changed;
    };

    /*
    */


    Model.prototype.save = function(next) {
      if (next == null) {
        next = function() {};
      }
      return this.schema.save(new Payload(this), next);
    };

    /*
    */


    Model.prototype.bind = function(property) {
      var binding;

      binding = Model.__super__.bind.apply(this, arguments);
      if (this._ignoreFetch) {
        return binding;
      }
      this._fetch(property);
      return binding;
    };

    /*
    */


    Model.prototype.validate = function(next) {
      var error;

      error = this.schema.validate(this);
      if (arguments.length === 1) {
        return next(error);
      } else {
        return error;
      }
    };

    /*
    */


    Model.prototype._clone = function(data) {
      return JSON.parse(JSON.stringify(data));
    };

    /*
     refreshes the model
    */


    Model.prototype.fetch = function(next) {
      return this.schema.fill(this, [], next);
    };

    /*
    */


    Model.prototype._fetch = function(property) {
      return this.schema.fetch(this, [property]);
    };

    /*
    */


    Model.prototype._bindFields = function() {
      var fieldName, _fn, _i, _len, _ref,
        _this = this;

      this._ignoreFetch = true;
      _ref = this.schema.fields.names();
      _fn = function(fieldName) {
        var field, property, _j, _len1, _ref1, _results;

        field = _this.schema.fields.get(fieldName);
        _this.bind(fieldName).to(function(newValue, oldValue) {
          _this._changed[fieldName] = {
            key: fieldName,
            nv: newValue,
            ov: oldValue
          };
          if (field.set) {
            return field.set(_this, newValue, oldValue);
          }
        });
        if (field.get) {
          _this.set(field.property, field.get(_this));
        }
        if (field.bind) {
          _ref1 = field.bind;
          _results = [];
          for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
            property = _ref1[_j];
            _results.push((function(property) {
              return _this.bind(property).to(function() {
                return _this.set(field.property, field.get(_this));
              });
            })(property));
          }
          return _results;
        }
      };
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        fieldName = _ref[_i];
        _fn(fieldName);
      }
      return this._ignoreFetch = false;
    };

    return Model;

  })(bindable.Object);

  module.exports = Model;

}).call(this);
