  ...
  Transport = (function(_super) {
    ...
    */
    ...
    Transport.prototype.request = function(options, next) {
      ...
      return options.model._memos.call(currentHash = this._payloadHash(options), this._memoOps, next, function(next) {
        ...
        if (!method) {
          return next(new Error("cannot \"" + (options.method.toUpperCase()) + "\" \"" + _this.field.path + "\""));
        }
        return method(options, function(err, result) {
          if (result == null) {
            result = {};
          }
    ...
    */
    ...
    Transport.prototype._getPayloadData = function(payload) {
      var d, dataFields, field, model, _i, _len, _results;
      ...
      for (_i = 0, _len = dataFields.length; _i < _len; _i++) {
        field = dataFields[_i];
        _results.push(dref.set(d, field.path, field._mapper.normalize(model, model.get(field.path))));
      }
      return _results;
    };
